...
PACKAGENAME = metadata.get('package_name', 'jwedb')
DESCRIPTION = metadata.get('description', 'Python interface to the JWST DMS Engineering Database')
AUTHOR = metadata.get('author', 'Space Telescope Science Institute')
AUTHOR_EMAIL = metadata.get('author_email', '')
LICENSE = metadata.get('license', 'unknown')
URL = metadata.get('url', 'https://github.com/spacetelescope/jwst-dms-edb')
__minimum_python_version__ = metadata.get("minimum_python_version", "3.5")

# Enforce Python version check - this is the same check as in __init__.py but
# this one has to happen before importing ah_bootstrap.
if sys.version_info < tuple((int(val) for val in __minimum_python_version__.split('.'))):
    sys.stderr.write("ERROR: jwedb requires Python {} or later\n".format(__minimum_python_version__))
    sys.exit(1)
